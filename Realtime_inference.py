...
def classify_sound(file_name):
    ...
    #( '\n'.join('  {:12s}: {:.3f}'.format(yamnet_classes[i], prediction[i])
                    #for i in top5_i))
    for i in top5_i:
        print(yamnet_classes[i],print(prediction[i]))
        if('Speech' in yamnet_classes[i] and prediction[i]>0.50):
            return yamnet_classes[i]
...
while(True):
    ...
    pred = classify_sound(FILE_NAME)
    print(pred,lastcommand)
        
    if("Speech" in pred):
        lastcommand=1
        print('inside speech')
        if(lastcommand==1):
            toaster.show_toast("Alert","Someone is speaking!!!")
